...
Clients go through a KYC check, receive a chain-agnostic Wallet, and can deposit or withdraw funds across
multiple fiat-backed digital asset types (e.g., USD, EUR) on one or more Networks — with the corresponding
tokens being minted or burned on-chain. Off-chain state is persisted in Google Firestore. The system is
...
**User Story:** As a Client, I want to view my current Deposit_Token balance per (Asset_Type, Network) and my full transaction history, so that I can track my deposits and withdrawals across all asset types and networks.
...
3. THE Backend_API SHALL expose an endpoint that returns the transaction history for a Client by reading records from Firestore, with each record including the Asset_Type and Network.
4. THE Frontend SHALL display the Client's current Deposit_Token balance per (Asset_Type, Network) and a list of past deposit and withdrawal transactions, each labelled with its Asset_Type and Network.
5. WHEN a new deposit or withdrawal transaction is completed, THE Backend_API SHALL update Firestore so that the transaction appears in the Client's history within 5 seconds of on-chain confirmation.
...
2. WHEN a Bank_Operator triggers a pause on a Smart_Contract for a given (Asset_Type, Network) pair, THE Smart_Contract SHALL halt all minting and burning operations for that (Asset_Type, Network) until unpaused.
3. WHILE a Smart_Contract is paused, THE Backend_API SHALL reject deposit and withdrawal requests for the corresponding (Asset_Type, Network) pair and return a descriptive error to the Frontend.
...
1. WHEN a Mint or Burn event is emitted by THE Smart_Contract for any (Asset_Type, Network) pair, THE Backend_API SHALL write a corresponding record to Firestore containing the Chain_Address, Asset_Type, Network, amount, transaction hash, and timestamp.
